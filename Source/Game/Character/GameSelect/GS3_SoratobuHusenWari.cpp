#include "GS3_SoratobuHusenWari.h"
#include <imgui.h>
...
// コンストラクタ
GS3_SoratobuHusenWari::GS3_SoratobuHusenWari()
{
...
// 更新処理
void GS3_SoratobuHusenWari::Update(float elapsedTime)
{
	// ステート毎の更新処理
	switch (state)
	{
...
// 描画処理
void GS3_SoratobuHusenWari::Render(ID3D11DeviceContext* dc, Shader* shader)
{
	shader->Draw(dc, model.get(), materialColor, opacity, emissiveColor, emissiveStrength);
}

// HPなどの描画
void GS3_SoratobuHusenWari::SpriteRender(ID3D11DeviceContext* dc, Graphics& graphics)
{
}

// デバッグプリミティブ描画
void GS3_SoratobuHusenWari::DrawDebugPrimitive()
{
	// 基底クラスのデバッグプリミティブ描画
...
// 縄張り設定
void GS3_SoratobuHusenWari::SetTerritory(const DirectX::XMFLOAT3& origin, float range)
{
...
// ターゲット位置をランダム設定
void GS3_SoratobuHusenWari::SetRandomTargetPosition()
{
	// 縄張り範囲内でランダムな位置を生成
...
// 目標地点へ移動
void GS3_SoratobuHusenWari::MoveToTarget(float elapsedTime, float speedRate)
{
	// ターゲット方向への進行ベクトルを算出
...
// 待機ステートへ遷移
void GS3_SoratobuHusenWari::TransitionWaitState()
{
	state = State::Wait;
...
// 待機ステート更新処理
void GS3_SoratobuHusenWari::UpdateWaitState(float elapsedTime)
{
}

// プレイヤーとの接触処理
void GS3_SoratobuHusenWari::CollisionVsPlayer()
{
