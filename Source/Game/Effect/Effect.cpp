...
{
	// エフェクトを読み込みする前にロックする
	// ※マルチスレッドでEffectを作成するとDeviceContextを同時にアクセスして
	// フリーズする可能性があるので排他制御する
	std::lock_guard<std::mutex>lock(Graphics::Instance().GetMutex());

	// Effekseerのリソースを読み込む
	// EffekseerはUTF-16のファイルパス意外は対応していないため文字コード変換が必要
...
// 再生
Effekseer::Handle Effect::Play(const DirectX::XMFLOAT3& position, const DirectX::XMFLOAT3 scale, const DirectX::XMFLOAT3 angle)
{
	Effekseer::ManagerRef effekseerManager = EffectManager::Instance().GetEffekseerManager();
