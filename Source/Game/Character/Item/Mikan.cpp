...
Mikan::Mikan()
{
	model = std::make_unique<Model>("Data/Model/Item/Mikan/Mikan.mdl");

	getEffect = std::make_unique <Effect>("Data/Effect/Effect/Mikan.efk");
...
// 更新処理
void Mikan::Update(float elapsedTime)
{
	position.y = 0.8f;
...
// 描画処理
void Mikan::Render(ID3D11DeviceContext* dc, Shader* shader)
{
	shader->Draw(dc, model.get());
}

// HPなどの描画
void Mikan::SpriteRender(ID3D11DeviceContext* dc)
{

}

// デバッグ用GUI描画
void Mikan::DrawDebugGUI()
{
	if (ImGui::TreeNode("Mikan"))
	{
...
// プレイヤーとの接触処理
void Mikan::CollisionItemVsPlayer()
{
	Player& player = Player::Instance();
...
// 待機ステートへ遷移
void Mikan::TransitionInitState()
{
	state = State::Init;
...
// 待機ステート更新処理
void Mikan::UpdateInitState(float elapsedTime)
{
}