...
void CameraController::UpdateCameraTarget(float elapsedTime)
{
	...
	if (tracking)
	{
		...
		// ターゲット（敵）の位置を取得
		DirectX::XMFLOAT3 targetPos = target;

		// カメラの位置を計算
		DirectX::XMFLOAT3 cameraPosition;
		cameraPosition.x = playerPos.x - range * cosf(angle.y);
		cameraPosition.y = playerPos.y + height;
		cameraPosition.z = playerPos.z - range * sinf(angle.y);

		// プレイヤーから敵への方向ベクトルを計算
		DirectX::XMVECTOR playerPosVec = DirectX::XMLoadFloat3(&playerPos);
		DirectX::XMVECTOR targetPosVec = DirectX::XMLoadFloat3(&targetPos);
		DirectX::XMVECTOR directionVec = DirectX::XMVectorSubtract(targetPosVec, playerPosVec);
		directionVec = DirectX::XMVector3Normalize(directionVec);

		// カメラの回転行列を計算
		angle.y = atan2f(DirectX::XMVectorGetX(directionVec), DirectX::XMVectorGetZ(directionVec));
		angle.x = -asinf(DirectX::XMVectorGetY(directionVec));

		// ターゲットがプレイヤーより下の場合、縦方向の回転を制限
		...
		// 回転行列を計算
		DirectX::XMMATRIX rotationMatrix = DirectX::XMMatrixRotationRollPitchYaw(angle.x, angle.y, 0.0f);
		DirectX::XMVECTOR offsetVec = DirectX::XMLoadFloat3(&DirectX::XMFLOAT3(0.0f, height, -range));
		DirectX::XMVECTOR rotatedOffsetVec = DirectX::XMVector3Transform(offsetVec, rotationMatrix);
		DirectX::XMVECTOR cameraPosVec = DirectX::XMVectorAdd(playerPosVec, rotatedOffsetVec);
		DirectX::XMStoreFloat3(&cameraPosition, cameraPosVec);

		// カメラの視点と注視点を設定
		Camera::Instance().SetLookAt(cameraPosition, playerPos, DirectX::XMFLOAT3(0.0f, 1.0f, 0.0f));

		// プレイヤーとターゲットの距離を求める
		DirectX::XMVECTOR playerVec = DirectX::XMLoadFloat3(&playerPos);
		DirectX::XMVECTOR targetVec = DirectX::XMLoadFloat3(&target);
		DirectX::XMVECTOR distanceVec = DirectX::XMVectorSubtract(targetVec, playerVec);

		DirectX::XMVECTOR distanceLength = DirectX::XMVector3Length(distanceVec);

		// 結果を取得
		DirectX::XMStoreFloat(&dist, distanceLength);

		// 遠くにいる時、カメラの高さを下げる
		if ((dist > 1900 || target.y > 4.0f) && height > 6.5f)
		{
			height -= 4 * elapsedTime;
		}
		else if (dist <= 1900 && height < 9.0f)
		{
			height += 4 * elapsedTime;
		}
	}
