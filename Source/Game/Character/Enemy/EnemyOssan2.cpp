...
// 更新処理
void EnemyOssan2::Update(float elapsedTime)
{
	// ステート毎の更新処理
	switch (state)
	{
...
// 描画処理
void EnemyOssan2::Render(ID3D11DeviceContext* dc, Shader* shader)
{
	shader->Draw(dc, model.get(), materialColor, opacity);
}

// HPなどの描画
void EnemyOssan2::SpriteRender(ID3D11DeviceContext* dc, Graphics& graphics)
{
}

// デバッグプリミティブ描画
void EnemyOssan2::DrawDebugPrimitive()
{
	// 基底クラスのデバッグプリミティブ描画
...
void EnemyOssan2::DrawDebugGUI()
{
	if (ImGui::TreeNode("EnemyOssan2"))
	{
...
// ダメージ受けた時に呼ばれる
void EnemyOssan2::OnDamaged()
{
}

// 死亡しと時に呼ばれる
void EnemyOssan2::OnDead()
{
}

// 上昇ステートへ遷移
void EnemyOssan2::TransitionUpState()
{
	state = State::Up;
...
// 上昇ステート更新処理
void EnemyOssan2::UpdateUpState(float elapsedTime)
{
	//! 位置Yの変更が終わったら
	if (!positionChange.y)
	{
...
// 下降ステートへ遷移
void EnemyOssan2::TransitionDownState()
{
	state = State::Down;
...
// 下降ステートへ遷移
void EnemyOssan2::UpdateDownState(float elapsedTime)
{
	//! 位置Yの変更が終わったら
	if (!positionChange.y)
	{
...
// ダメージステートへ遷移
void EnemyOssan2::TransitionDamageState()
{
	state = State::Damage;
...
// ダメージステート更新処理
void EnemyOssan2::UpdateDamageState(float elapsedTime)
{
