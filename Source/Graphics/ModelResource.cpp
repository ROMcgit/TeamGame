...
void ModelResource::BuildModel(ID3D11Device* device, const char* dirname)
{
	for (Material& material : materials)
	{
...
#if 1
		{
			// 相対パスの解決
			char filenameN[256];
			std::string NName = material.textureFilename + "_normal";
			::_makepath_s(filenameN, 256, nullptr, dirname, NName.c_str(), nullptr);
...
#if 0
		{
			// 相対パスの解決
			char filenameN[256];
			std::string NName = "R" + material.textureFilename;
			::_makepath_s(filenameN, 256, nullptr, dirname, NName.c_str(), nullptr);
			...
			// テクスチャ読み込み
			Microsoft::WRL::ComPtr<ID3D11Resource> resource;
			HRESULT hr = DirectX::CreateWICTextureFromFile(device, wfilename, resource.GetAddressOf(), material.SRV_Roughness.GetAddressOf());
			if (FAILED(hr))
			{
				// WICでサポートされていないフォーマットの場合（TGAなど）は
				// STBで画像読み込みをしてテクスチャを生成する
				...
				if (pixels != nullptr)
				{
					...
					hr = device->CreateShaderResourceView(texture.Get(), nullptr, material.SRV_Roughness.GetAddressOf());
					_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
...
#if 0
		{
			// 相対パスの解決
			char filenameN[256];
			std::string NName = "M" + material.textureFilename;
			::_makepath_s(filenameN, 256, nullptr, dirname, NName.c_str(), nullptr);
			...
			// テクスチャ読み込み
			Microsoft::WRL::ComPtr<ID3D11Resource> resource;
			HRESULT hr = DirectX::CreateWICTextureFromFile(device, wfilename, resource.GetAddressOf(), material.SRV_Metallic.GetAddressOf());
			if (FAILED(hr))
			{
				// WICでサポートされていないフォーマットの場合（TGAなど）は
				// STBで画像読み込みをしてテクスチャを生成する
				...
				if (pixels != nullptr)
				{
					...
					hr = device->CreateShaderResourceView(texture.Get(), nullptr, material.SRV_Metallic.GetAddressOf());
					_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
