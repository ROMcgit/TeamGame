...
ShadowMapShader::ShadowMapShader(ID3D11Device* device)
{
	...
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\ShadowMapVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\ShadowMapPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
void ShadowMapShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
	...
	cbScene.lightRangeConst = rc.lightRange;       // ライトの範囲

	cbScene.ambientStrengthConst  = rc.ambientStrength;  // 環境光
	cbScene.diffuseStrengthConst  = rc.diffuseStrength;  // 拡散光
	cbScene.specularStrengthConst = rc.specularStrength; // スペキュラー光
	cbScene.lightPadding          = 0.0f;                // ライトのパディング

	cbScene.fogColorConst = rc.fogColor;         // フォグの色
	cbScene.fogStartConst = rc.fogStart;         // フォグの開始
	cbScene.fogEndConst   = rc.fogEnd;           // フォグの終了
...
// 描画
void ShadowMapShader::Draw(ID3D11DeviceContext* dc, const Model* model, DirectX::XMFLOAT3 materialColor, float opacity,
