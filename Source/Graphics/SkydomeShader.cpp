...
#include "Graphics/SkydomeShader.h"
...
SkydomeShader::SkydomeShader(ID3D11Device* device)
{
	...
	// 定数バッファ
	{
		...
		// スカイマップの大きさ用バッファ
		desc.ByteWidth = sizeof(Scale);
		hr = device->CreateBuffer(&desc, 0, scaleConstantBuffer.GetAddressOf());
...
// 描画開始
void SkydomeShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
	...
	cbScene.cameraPosition = DirectX::XMFLOAT4(CameraController::target.x, CameraController::target.y, CameraController::target.z, 0.0f);

	dc->UpdateSubresource(sceneConstantBuffer.Get(), 0, 0, &cbScene, 0, 0);

	// スカイマップの大きさ
	Scale scale;
	scale.skydomePosition = rc.skydomePosition;
	scale.skydomeAngle    = rc.skydomeAngle;
	scale.skydomeScale    = rc.skydomeScale;

	scale.dummy = 0;

	dc->UpdateSubresource(scaleConstantBuffer.Get(), 0, 0, &scale, 0, 0);
}

// 描画
void SkydomeShader::Draw(ID3D11DeviceContext* dc, const Model* model, DirectX::XMFLOAT3 materialColor, float opacity,
...
// 描画終了
void SkydomeShader::End(ID3D11DeviceContext* dc)
{
	// シェーダー設定解除
